...
2. displayTypeAndArea()
	2-1. displayTypeAndArea() displays the type (either circle or rectangle) and the area (as a float value) of the corresponding object.
...
public:
    ...
    static int cnt;
    int operator>(const Shape &shape0){
        Shape shape1;
       if ( shape0.area > shape1.area){
           return 1;
       } else if(shape0.area == shape1.area){
           return -1;
       } else {
           return 0;
       }
...
public:
    explicit Rectangle(int i) : Shape(){
        _d = i;
        area = _d * _d;
    }

    void displayTypeAndArea() override{
        cout << "Type: Rectangle" << "Area:" << area << endl;
    };
};
